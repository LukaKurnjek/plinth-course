...
zero and increasing by one. Because a transaction hash(((transaction, hash))) (ID) is unique,
...
As we explained in the previous section, we need to define the
...
reference(((transaction, output reference))) and the [purple]#Maybe Datum# type. We use
...
an address has
two parts: payment part and optional staking. The payment part,
defined with the [purple]#Credential# data type, contains either a public
key hash or a script hash, as we explained when discussing rewarding
...
outputs(((transaction, output))) that the
...
reference(((transaction, output reference))), without the [purple]#Maybe Datum# type.
...
Next are the votes and proposal procedures used when
