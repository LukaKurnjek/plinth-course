...
allows code-sharing between the on- and off-chain components while
enabling the creation of a backend for DApps.
...
contract `vestingVal` that we defined in the previous section. Our code
...
present in this and other sections was tested on the _preview_ network.
