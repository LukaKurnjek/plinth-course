...
https://www.essentialcardano.io/article/a-list-of-community-built-developer-tools-on-cardano[developer tools] list hosted on Essential Cardano.

This lesson explores the https://meshjs.dev/[MeshJS] tool, developed by
...
other lessons, you can use https://deno.com/[Deno] – which is a runtime
...
see Mastering Cardano chapter
https://github.com/input-output-hk/mastering-cardano/blob/main/chapters/chapter-06-wallets-in-the-world-of-cardano.adoc[Cardano digital wallets].
...
https://meshjs.dev/providers[Providers] documentation page.
...
the datum the same way we have shown in the [blue]#sendFunds# function. For
...
instead of [blue]#mConStr0#. With it, we create a Mesh Data index one constructor object
...
MeshJS docs lesson that also showcases a code example.

If the redeemer does not represent a custom data type,
but rather a Plutus supported type as eg an [purple]#Integer#, we would define
the redeemer as:
...
that we call with five ada, and the [blue]#claimFunds# function that
...
We now use the function [blue]#applyParamsToScript# and add the
...
Cardano section
https://github.com/input-output-hk/mastering-cardano/blob/main/chapters/chapter-04-how-cardano-works/cardano-node-and-system-layers.adoc#consensus-and-storage-layer[Consensus and storage layer]). Once you have written on-chain code that works,
